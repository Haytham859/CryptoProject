#include "aes.h"
#include <QByteArray>
#include <QCryptographicHash>
#include <cstring>

// -------------------- S-boxes and constants --------------------
static const unsigned char sbox[256] = {
    
};

static const unsigned char inv_sbox[256] = {
    
};

static const unsigned char Rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

// multiply in GF(2^8)
static unsigned char xtime(unsigned char x) {
    return (unsigned char)((x << 1) ^ ((x & 0x80) ? 0x1B : 0x00));
}
static unsigned char mul(unsigned char a, unsigned char b) {
    unsigned char res = 0;
    while (b) {
        if (b & 1) res ^= a;
        a = xtime(a);
        b >>= 1;
    }
    return res;
}

// ----------------- Key expansion (AES-128) -----------------
static void KeyExpansion(const unsigned char key[16], unsigned char roundKey[176]) {
    // first round key is the key itself
    for (int i = 0; i < 16; i++) roundKey[i] = key[i];

    int bytesGenerated = 16;
    int rconIter = 1;
    unsigned char temp[4];

    while (bytesGenerated < 176) {
        for (int i = 0; i < 4; i++)
            temp[i] = roundKey[bytesGenerated - 4 + i];

        if (bytesGenerated % 16 == 0) {
            // rotate
            unsigned char t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;
            // sub
            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];
            // Rcon
            temp[0] ^= Rcon[rconIter];
            rconIter++;
        }

        for (int i = 0; i < 4; i++) {
            roundKey[bytesGenerated] = roundKey[bytesGenerated - 16] ^ temp[i];
            bytesGenerated++;
        }
    }
}

// ----------------- Core AES primitives -----------------
static void SubBytes(unsigned char state[16]) {
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
}
static void InvSubBytes(unsigned char state[16]) {
    for (int i = 0; i < 16; i++) state[i] = inv_sbox[state[i]];
}
static void ShiftRows(unsigned char state[16]) {
    unsigned char tmp[16];
    tmp[0]=state[0]; tmp[1]=state[5]; tmp[2]=state[10]; tmp[3]=state[15];
    tmp[4]=state[4]; tmp[5]=state[9]; tmp[6]=state[14]; tmp[7]=state[3];
    tmp[8]=state[8]; tmp[9]=state[13]; tmp[10]=state[2]; tmp[11]=state[7];
    tmp[12]=state[12]; tmp[13]=state[1]; tmp[14]=state[6]; tmp[15]=state[11];
    memcpy(state,tmp,16);
}
static void InvShiftRows(unsigned char state[16]) {
    unsigned char tmp[16];
    tmp[0]=state[0]; tmp[1]=state[13]; tmp[2]=state[10]; tmp[3]=state[7];
    tmp[4]=state[4]; tmp[5]=state[1]; tmp[6]=state[14]; tmp[7]=state[11];
    tmp[8]=state[8]; tmp[9]=state[5]; tmp[10]=state[2]; tmp[11]=state[15];
    tmp[12]=state[12]; tmp[13]=state[9]; tmp[14]=state[6]; tmp[15]=state[3];
    memcpy(state,tmp,16);
}
static void MixColumns(unsigned char state[16]) {
    for (int c=0;c<4;c++){
        int i = c*4;
        unsigned char a0 = state[i+0], a1 = state[i+1], a2 = state[i+2], a3 = state[i+3];
        unsigned char r0 = (unsigned char)(mul(0x02,a0) ^ mul(0x03,a1) ^ a2 ^ a3);
        unsigned char r1 = (unsigned char)(a0 ^ mul(0x02,a1) ^ mul(0x03,a2) ^ a3);
        unsigned char r2 = (unsigned char)(a0 ^ a1 ^ mul(0x02,a2) ^ mul(0x03,a3));
        unsigned char r3 = (unsigned char)(mul(0x03,a0) ^ a1 ^ a2 ^ mul(0x02,a3));
        state[i+0]=r0; state[i+1]=r1; state[i+2]=r2; state[i+3]=r3;
    }
}
static void InvMixColumns(unsigned char state[16]) {
    for (int c=0;c<4;c++){
        int i = c*4;
        unsigned char a0 = state[i+0], a1 = state[i+1], a2 = state[i+2], a3 = state[i+3];
        unsigned char r0 = (unsigned char)(mul(0x0e,a0) ^ mul(0x0b,a1) ^ mul(0x0d,a2) ^ mul(0x09,a3));
        unsigned char r1 = (unsigned char)(mul(0x09,a0) ^ mul(0x0e,a1) ^ mul(0x0b,a2) ^ mul(0x0d,a3));
        unsigned char r2 = (unsigned char)(mul(0x0d,a0) ^ mul(0x09,a1) ^ mul(0x0e,a2) ^ mul(0x0b,a3));
        unsigned char r3 = (unsigned char)(mul(0x0b,a0) ^ mul(0x0d,a1) ^ mul(0x09,a2) ^ mul(0x0e,a3));
        state[i+0]=r0; state[i+1]=r1; state[i+2]=r2; state[i+3]=r3;
    }
}
static void AddRoundKey(unsigned char state[16], const unsigned char *roundKey) {
    for (int i=0;i<16;i++) state[i] ^= roundKey[i];
}

// ----------------- Encrypt / Decrypt one block -----------------
static void encryptBlock(const unsigned char in[16], unsigned char out[16], const unsigned char roundKey[176]) {
    unsigned char state[16];
    memcpy(state, in, 16);

    AddRoundKey(state, roundKey);

    for (int round=1; round<=9; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, roundKey + round*16);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, roundKey + 160);

    memcpy(out, state, 16);
}

static void decryptBlock(const unsigned char in[16], unsigned char out[16], const unsigned char roundKey[176]) {
    unsigned char state[16];
    memcpy(state, in, 16);

    // initial
    AddRoundKey(state, roundKey + 160);
    // 9 rounds
    for (int round=9; round>=1; round--) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, roundKey + round*16);
        InvMixColumns(state);
    }
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, roundKey);

    memcpy(out, state, 16);
}

// ----------------- Padding PKCS7 (block 16) -----------------
static QByteArray addPadding(const QByteArray &data) {
    int pad = 16 - (data.size() % 16);
    QByteArray copy = data;
    for (int i=0;i<pad;i++) copy.append(static_cast<char>(pad));
    return copy;
}
static QByteArray removePadding(const QByteArray &data) {
    if (data.isEmpty()) return QByteArray();
    int pad = static_cast<unsigned char>(data.constData()[data.size()-1]);
    if (pad < 1 || pad > 16) return data; // safety: if padding invalid, return raw
    return data.left(data.size() - pad);
}

// ----------------- Key derivation from text -----------------
static void keyFromText(const QString &keyText, unsigned char keyOut[16]) {
    QByteArray hash = QCryptographicHash::hash(keyText.toUtf8(), QCryptographicHash::Md5); // 16 بايت
    memcpy(keyOut, hash.constData(), 16);
}



// ----------------- Public API -----------------
QString aesEncryptText(const QString &plainText, const QString &keyText) {
    unsigned char key[16];
    keyFromText(keyText, key); // هنا بناخد المفتاح من نص عادي

    unsigned char roundKey[176];
    KeyExpansion(key, roundKey);

    QByteArray data = addPadding(plainText.toUtf8());
    QByteArray out;
    out.resize(data.size());

    for (int i=0;i<data.size(); i+=16) {
        unsigned char inb[16], outb[16];
        memcpy(inb, reinterpret_cast<const unsigned char*>(data.constData()) + i, 16);
        encryptBlock(inb, outb, roundKey);
        memcpy(reinterpret_cast<unsigned char*>(out.data())+i, outb, 16);
    }

    // تحويل كل بايت لسلسلة 0 و 1
    QString binStr;
    for (auto c : out) {
        binStr += QString("%1").arg(static_cast<unsigned char>(c), 8, 2, QChar('0'));
    }
    return binStr;
}
QString aesDecryptText(const QString &cipherBin, const QString &keyText) {
    // تحويل الـ binary string لـ QByteArray
    QByteArray cipher;
    if(cipherBin.size() % 8 != 0) return QString(); // invalid
    for(int i=0;i<cipherBin.size();i+=8){
        bool ok;
        char byte = static_cast<char>(cipherBin.mid(i,8).toUInt(&ok,2));
        if(!ok) return QString();
        cipher.append(byte);
    }

    unsigned char key[16];
    keyFromText(keyText, key); // استخدم نفس الطريقة هنا

    unsigned char roundKey[176];
    KeyExpansion(key, roundKey);

    QByteArray out;
    out.resize(cipher.size());
    for (int i=0;i<cipher.size(); i+=16) {
        unsigned char inb[16], outb[16];
        memcpy(inb, reinterpret_cast<const unsigned char*>(cipher.constData()) + i, 16);
        decryptBlock(inb, outb, roundKey);
        memcpy(reinterpret_cast<unsigned char*>(out.data())+i, outb, 16);
    }
    QByteArray plain = removePadding(out);
    return QString::fromUtf8(plain);
}
